...
- **Home sliders:** Righe orizzontali con scroll snap (`pm-slider`): «Continua a guardare» e, se presente almeno un titolo salvato, **Watchlist** subito dopo; poi «Aggiunti di recente» con card poster 2:3 (`w-36`/`w-44`); sotto compaiono righe per genere (titolo = nome genere, slider contenuti classificati) solo se il genere ha almeno un titolo, in ordine alfabetico, con link «Vedi tutto» al catalogo filtrato. Track senza scrollbar visibile, scroll touch (`pan-x`) e drag col mouse su desktop; la rotella scrolla solo la pagina (non gli slider); frecce prev/next (`pm-slider__nav`). Pulsante bookmark (`pm-poster-bookmark`) in alto a sinistra sui poster home/catalogo: outline semi-trasparente, visibile al hover (sempre visibile se attivo o su touch); stato attivo con icona `bookmark` filled e accento `secondary` (`#ffe083`).
...
- **Header app:** Menu lingua in alto a destra (`pm-locale-menu`): trigger pill con icona `language`, codice locale (`IT`/`EN`) e chevron; pannello glass con opzioni native e check sulla lingua attiva. Posizionato prima del badge utente. Stessa UI sulla shell login (angolo alto dx). **Utenti non-admin:** icona `settings` standalone (`pm-header-settings`, pill arrotondata hover) tra lingua e badge utente, fuori dal pill nick/logout; su mobile visibile in header accanto al burger (non nel drawer). **Mobile (< md):** burger in header; drawer full-screen (`bg-background`, stesso body) con pulsante `close` in alto a destra, link navigazione con icona Material, footer con lingua in dropdown e voce esci in listing (icona `logout` + nome utente + «Esci»).
- **Account settings (non-admin):** Shell con sidebar sinistra come admin (`/account`, `/account/dispositivi`, `/account/contenuti`). Menu: Impostazioni (lingua), Dispositivi collegati, **Contenuti** (filtro librerie put.io: checkbox libreria principale + amici con contenuti sincronizzati; righe selezionate stile `pm-friend-row--selected`). Breadcrumb «Account › …».
- **Dispositivi collegati:** Lista condivisa (`devices-list` partial) per account e admin. Card `surface-container` per dispositivo: icona tipo, label, IP, ultimo accesso, scadenza; badge primary «Questo dispositivo» se corrente; pulsante Rimuovi con conferma. Stato vuoto con icona `devices_off`. Admin: voce menu sidebar `devices` → `/admin/dispositivi`.
- **Filtro contenuti utente:** Preferenze in `user_catalog_hidden_sources` (default: tutto visibile). Checkbox con salvataggio AJAX al toggle (`POST /api/account/catalog-sources`, toast come amici put.io). Applicate a Home, Catalogo, In corso, Watchlist, API catalogo, dettaglio media, player e stream. Admin sempre vede tutto.
- **Watchlist:** Lista personale per-utente (`user_watchlist`) accessibile da menu **Watchlist** (`/watchlist`) e slider home condizionale. Bookmark su poster (home/catalogo) e pagina dettaglio; toggle via `POST /api/watchlist`. Solo titoli principali (film e serie, non episodi singoli). Barra progresso opzionale sulle card watchlist se il titolo è anche in corso.
- **Player:** Usa l'header app condiviso (glass + nav). Contenuto max-width 1200px: video Video.js in modalità fluid con aspect ratio dinamico dai metadati (placeholder 16:9), artwork player con **backdrop TMDB** (`backdrop_local_path` / `backdrop_url`, w1280) e fallback poster; sfondo cinematico sfocato (`.putmio-player-poster-bg`) + `poster` nativo Video.js, nascosto in riproduzione. Overlay caricamento stream (`.putmio-player-wrap--loading`: velo scuro + spinner Video.js) su tutti i dispositivi fino all'avvio riproduzione. Per episodi fallback backdrop/poster della serie. Bordo `outline-variant/30`, selettore sorgente MP4/originale, traccia audio (se disponibili) e **sottotitoli** via menu CC nativo Video.js (tutte le tracce scaricate), pulsante «Gestisci sottotitoli» + hint testuale, pannello sync ±0.1s/±0.5s (offset server-side sul VTT) visibile quando una traccia è attiva. Aspetto sottotitoli via modale nativa Video.js (CC → impostazioni): default font 100%, testo bianco, sfondo `#060e20`, bordo uniforme; preferenze salvate in localStorage utente. Modale stilizzata PutMio (`surface-container-high` / `#222a3d` sui select). Posizione cue ancorata in basso sopra control bar. Modale sottotitoli glass max-width 800px: sezione «Già disponibili» (card `surface-container-high`, Usa/Elimina admin) + «Cerca su OpenSubtitles» con risultati (lingua, release, download count). Nei risultati ricerca: «Scarica» attivo, «Scaricamento…» (disabled + spinner) in corso, «Già scaricato» (disabled, bordo/icona success) se presente in cache o appena scaricato; toast di conferma al termine. Footer attribuzione OpenSubtitles. Nota «Streaming da put.io», card info con titolo (serie per episodi), sottotitolo monospaced primary, durata/anno, synopsis, azioni Riprendi/Riproduci · Segna come visto · Ricomincia · prev/next episodio, footer tecnico da nome file. Sfondo atmosferico con blob primary/tertiary. Controlli Video.js con skip nativi ±10s nella control bar, volume verticale (hover sul mute: pannello `surface-container` arrotondato 12px, track pill) e progress bar `primary-container` (`#8083ff`). Toast `.pm-next-episode` in overlay (ultimi 30 s).
...
- **Versioning asset (cache-busting):** Riferire **sempre** CSS/JS locali tramite l'helper `putmio_asset('public/assets/<file>')`, mai con URL hardcoded. L'helper inserisce la versione (`filemtime`) nel **nome del file** (es. `app.v1718900000.css`); una regola `mod_rewrite` in `.htaccess` rimappa il nome versionato al file reale. Quando il file cambia, cambia l'URL → tutti i client (incluse le Smart TV, dove svuotare la cache è scomodo) scaricano l'ultima versione. Il service worker (`sw.js`) **non** mette in precache CSS/JS (la cache-first runtime li salva per-URL versionato); precache solo per asset a nome stabile (icone, favicon, placeholder). Per aggiornare la cache degli asset a nome stabile, incrementare il numero in `CACHE` (`putmio-static-vN`) di `sw.js`. Dopo un aggiornamento core, il sync mirror su `public/` rimuove i vecchi `app.v*.css` / `*.v*.js` non più presenti nella release.
- **Admin sidebar:** Footer fisso in fondo al menu laterale (desktop): icona `info`, label «PutMio» monospaced uppercase, riga «Versione X.Y.Z» da file `VERSION` (`putmio_version()`). Bordo superiore `outline-variant/20`, testo `on-surface-variant`. Voci menu: Dashboard, Impostazioni, Classificazione, Streaming, Utenti, **Dispositivi collegati** (`devices`, hint login TV), Aggiornamenti (`system_update`).
- **Admin aggiornamenti:** Pagina `/admin/aggiornamenti` — due card glass (versione installata vs ultima release GitHub), changelog release, CTA «Aggiorna a X.Y.Z» (disabilitata se requisiti mancanti). Sezione espandibile «Cosa viene aggiornato»: elenco path protetti (`config.php`, `storage/`) vs path core (`src/`, `templates/`, `public/`, ecc.). Palette allineata: primary per versione locale, tertiary se update disponibile.
...
- **Compatibilità browser TV (LG webOS):** Classe `html.tv-ua` su UA Smart TV / webOS. Il browser integrato spesso non supporta `gap` su flex/grid (Chromium &lt; 84): **non affidarsi solo a utility Tailwind `gap-*`**. Pattern obbligatorio: margini di base sui figli + `@supports (gap: …)` per azzerarli sui browser moderni. Componenti riusabili (`.pm-slider__track`, `[data-catalog-grid]`) definiscono lo spacing in `app.css`. Fallback globale in `@supports not (gap: …)` per flex row/col, flex-wrap e grid. Reset `box-sizing: border-box` globale. Viewport rigido (`maximum-scale=1`) solo su UA TV.
- **IPK LG webOS (wrapper locale):** In `local-webos-app/` (fuori Git) l'IPK packaged 2.x inietta `tv-override.css` (font bundled, gap, tipografia 10-foot) e `tv-fix.js` (icone SVG) via proxy HTTP — palette allineata a questo file (`primary` `#c0c1ff`, `background` `#0b1326`, ecc.). Icone launcher `icon.png` / `largeIcon.png` replicate `favicon.svg` / PWA (`#c0c1ff` + play `#0d0096`). Non duplicare regole sul server; aggiornare l'IPK se si cambiano token critici TV o il marchio.
- **Sito TV `/tv/`:** Interfaccia 10-foot dedicata (`html.tv-site`), separata dal desktop/Tailwind. Asset in `public/assets/tv/` (`tv.css`, `spatial-nav.js`, vendor locale senza CDN). Layout `templates/tv/`, palette identica a questo file. Header fisso (`tv-header`: Home, Catalogo, In corso, Esci). Righe orizzontali Netflix (`tv-row`, poster ~200px, scroll al focus). Info rail in basso (`tv-info-rail`). Login solo QR (`/tv/login`): layout **due colonne** (testi + codice a sinistra, pannello QR fisso 280×280 a destra) per restare in viewport senza scroll. Focus virtuale `.tv-focused` + `data-tv-focus`. Tipografia min 20px, safe area 48–64px. CSP strict self-only su layout TV. Helper `putmio_tv_url()`, `putmio_tv_asset()`.