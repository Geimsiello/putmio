...
- **Header app:** Menu lingua in alto a destra (`pm-locale-menu`): trigger pill con icona `language`, codice locale (`IT`/`EN`) e chevron; pannello glass con opzioni native e check sulla lingua attiva. Posizionato prima del toggle tema e del badge utente. Stessa UI sulla shell login (angolo alto dx, accanto al tema).
