...
- **Login:** Shell centrata con blob atmosferici e toggle tema (guest). Sopra la card: logo `favicon.svg` + titolo PutMio + tagline (`body-md`). Card `auth-glass` con titolo sezione, barra accent primary, input con icona mail/lock (`auth-input`), pulsante `auth-btn-primary`, link password dimenticata. Autocomplete `username` / `current-password` per password manager.
- **Player:** Usa l'header app condiviso (glass + nav). Contenuto max-width 1200px: video `aspect-video` con bordo `outline-variant/30`, selettore sorgente MP4/originale, traccia audio (se disponibili) e **sottotitoli** (select Off + tracce, pulsante «Gestisci», pannello sync ±0.1s/±0.5s con input secondi e reset). Modale sottotitoli glass max-width 800px: sezione «Già disponibili» (card `surface-container-high`, Usa/Elimina admin) + «Cerca su OpenSubtitles» con risultati (lingua, release, download count). Footer attribuzione OpenSubtitles. Nota «Streaming da put.io», card info con titolo (serie per episodi), sottotitolo monospaced primary, durata/anno, synopsis, azioni Riprendi/Riproduci · Segna come visto · Ricomincia · prev/next episodio, footer tecnico da nome file. Sfondo atmosferico con blob primary/tertiary. Controlli Video.js con progress bar `primary-container`. Toast `.pm-next-episode` in overlay (ultimi 30 s).
