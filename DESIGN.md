...
- **Admin sidebar:** Footer fisso in fondo al menu laterale (desktop): icona `info`, label «PutMio» monospaced uppercase, riga «Versione X.Y.Z» da file `VERSION` (`putmio_version()`). Bordo superiore `outline-variant/20`, testo `on-surface-variant`. Voce menu **Aggiornamenti** (`system_update`) con hint «Versione core».
