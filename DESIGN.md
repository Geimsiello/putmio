...
- **Header app:** Menu lingua in alto a destra (`pm-locale-menu`): trigger pill con icona `language`, codice locale (`IT`/`EN`) e chevron; pannello glass con opzioni native e check sulla lingua attiva. Posizionato prima del toggle tema e del badge utente. Stessa UI sulla shell login (angolo alto dx, accanto al tema). **Mobile (< md):** burger in header + toggle tema; drawer full-screen (`bg-slate-50` / `dark:bg-background`, stesso body) con pulsante `close` in alto a destra (spazio per toggle tema), link navigazione con icona Material, footer con lingua in dropdown e voce esci in listing (icona `logout` + nome utente + «Esci»).
