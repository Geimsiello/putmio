...
- **Filtro contenuti utente:** Preferenze in `user_catalog_hidden_sources` (default: tutto visibile). Checkbox con salvataggio AJAX al toggle (`POST /api/account/catalog-sources`, toast come amici put.io). Applicate a Home, Catalogo, In corso, API catalogo, dettaglio media, player e stream. Admin sempre vede tutto.
...
- **TV mode (interfaccia 10-foot):** Attivazione auto su UA Smart TV / Android TV / Apple TV (`putmio_tv_mode()`) o toggle manuale (cookie/sessione `putmio_ui_mode`). Classe `html.tv-mode` su `<html>` in `layout.php`; header dedicato (`templates/partials/tv-header.php`, `pm-tv-header`: Home, Catalogo, In corso, Esci), navigazione telecomando (`tv-nav.js`, `data-pm-tv-focus`), pannello info al focus (`pm-tv-info-rail`), poster moderatamente più grandi, caption sotto poster nascoste (info nel rail). Play da TV: autostart + fullscreen automatico **solo su dispositivi TV** (`putmio-tv-player-immersive` CSS a schermo intero + tentativo API nativa), non su desktop. Admin/TMDB/OpenSubtitles modale disabilitati in TV mode (`putmio_admin_ui_enabled()`). Dettaglio media: layout hero `pm-catalog-tv-hero`. Home TV: wrapper `pm-home-tv-limit`, max 2 righe genere oltre a Continua/Recenti.
- **Compatibilità browser TV (LG webOS):** Classe `html.tv-ua` su UA Smart TV / webOS. Il browser integrato spesso non supporta `gap` su flex/grid (o lo dichiara in `@supports` senza renderizzarlo): **non affidarsi solo a utility Tailwind `gap-*`**. Pattern obbligatorio: margini su `html.tv-ua` (forzati, indipendenti da `@supports`) + `@supports not (gap: …)` come fallback aggiuntivo. Pulsanti secondari: classi `pm-btn-outline` in `app.css` (colori espliciti, non solo Tailwind). Componenti riusabili (`.pm-slider__track`, `[data-catalog-grid]`) definiscono lo spacing in `app.css`. Reset pulsanti/input nativi su `html.tv-ua`. Viewport rigido (`maximum-scale=1`) solo su UA TV.